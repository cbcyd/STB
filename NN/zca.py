...
def zca(content, style):
    ...
    u_c, sig_c, _ = torch.svd(cov_c + to_device(torch.eye(cov_c.size(0)))*1e-4)
    u_s, sig_s, _ = torch.svd(cov_s + to_device(torch.eye(cov_s.size(0)))*1e-4)
